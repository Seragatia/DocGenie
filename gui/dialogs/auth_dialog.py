...
class AuthorizationDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Tk, url: str = "", theme: Optional[Dict[str, str]] = None) -> None:
        logging.debug(f"Initializing AuthorizationDialog with URL: {url}")

        # Ensure only one AuthorizationDialog at a time.
        if AuthorizationDialog._active_dialog:
            logging.debug("An active AuthorizationDialog is already open. Focusing it.")
    ...
    def _create_widgets(self) -> None:
        ...
        lbl_address = ttk.Label(self.container, text="Address:")
        lbl_address.grid(row=0, column=0, sticky="e", padx=(0, 8), pady=(0, 8))
        ...
        # Row 1: Checkbutton for "Use authorization"
        ...
        # Row 2: Username
        lbl_username = ttk.Label(self.container, text="Username:")
        lbl_username.grid(row=2, column=0, sticky="e", padx=(0, 8), pady=4)
        ...
        # Row 3: Password
        lbl_password = ttk.Label(self.container, text="Password:")
        lbl_password.grid(row=3, column=0, sticky="e", padx=(0, 8), pady=4)
    ...
    def toggle_auth(self) -> None:
        ...
        logging.debug(f"Authorization checkbox toggled to {self.use_auth_var.get()}")
    ...
    def on_ok(self) -> None:
        ...
        logging.info(f"AuthorizationDialog result: {self.result}")
        self._close_dialog()
    ...
    def _close_dialog(self) -> None:
        ...
        if AuthorizationDialog._active_dialog is not self:
            return  # Ensure we don't close if not the active dialog
        AuthorizationDialog._active_dialog = None
        self.grab_release()
        self.destroy()
        self.parent.focus_force()
        logging.debug("AuthorizationDialog closed.")
