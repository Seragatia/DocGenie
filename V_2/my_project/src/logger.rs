...
use log::{info, error, warn, LevelFilter, Record, Metadata}; // Ensure these macros are imported from the log crate

/// Log levels for our logger.
#[derive(Debug, PartialEq, PartialOrd, Clone)] // Automatically derive Clone
pub enum LogLevel {
    Debug,
    Info,
    Warning,
...
impl Logger {
    ...
    pub fn init(config: LoggerConfig) -> Logger {
        ...
        log::set_max_level(match config.min_log_level {
    ...
    fn rotate_log(log_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let rotated_filename = format!("{}-{}.log", log_path.to_string_lossy(), timestamp);
        rename(log_path, rotated_filename)?;
        info!("Log file rotated: {}", rotated_filename);  // Log rotation action
        Ok(())
    }
}
